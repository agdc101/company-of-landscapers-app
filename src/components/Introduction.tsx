...
const Introduction = ({homePageData}: IIntroductionProps) => {
    ...
    return (
        <section className="flex flex-col items-center xl:flex-row justify-around py-20 px-2 xl:px-10 2xl:py-28">
            <div ref={introTextRef} className="text-center w-full lg:w-4/5 xl:w-2/5" 
                ...
                <h4 className="text-4xl 2xl:text-5xl">{homePageData.introTitle}</h4>
                <p className="text-slate-600 my-14 text-xl xl:my-20 xl:text-2xl 2xl:text-3xl">{homePageData.introDescription}</p>
            </div>
            <div className="xl:block px-4 md:w-3/6 max-w-screen-md">
                <img loading="lazy" ref={introImageRef} className="rounded shadow-custom" src={homePageData.introImage[0].url} alt={homePageData.introImage[0].alt}
                    style={{
                        transform: introTextIsInView ? "none" : "translateX(100px)",
                        opacity: introTextIsInView ? 1 : 0,
                        transition: "all 0.9s cubic-bezier(0.17, 0.55, 0.55, 1) 0.5s"
